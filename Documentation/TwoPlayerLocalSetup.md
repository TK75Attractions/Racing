...
- 1台のマイコンをUSB接続し、P1/P2の4列入力を改行区切りで送信できること（P1シリアル／P2キーボードの検証時はP1の先頭2列だけ有効でよい）
- PCにモニターを2台接続してからゲームを起動すること
- `SampleScene` の `InputManager > Is Debug Mode` は通常プレイでは無効にすること

現在の `SampleScene` はP1をESP32、P2をキーボードで操作する検証設定です。P2は上矢印を0.25秒押すか、Right Shiftを1回押すとタイトルで準備完了になります。両者が準備完了になるとレースが始まります。P1/P2ともESP32で操作する実機対戦では `Is P1 Serial P2 Keyboard Debug Mode` を無効にしてください。
